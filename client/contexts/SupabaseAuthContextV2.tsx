...
export function AuthProviderV2({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      try {
        console.log("[AuthV2] Starting initial session check...");
        ...
        console.log("[AuthV2] Checking Supabase session (with 5s timeout)...");
        ...
        try {
          ...
          sessionData = data;
          console.log("[AuthV2] Session check completed:", data);
        } catch (err) {
          console.warn("[AuthV2] Session check failed or timed out:", err);
          setUser(null);
          setUserUuid(null);
          setLoading(false);
          return;
