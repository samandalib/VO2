...
export default function RagAIAssistantHero() {
  ...
  async function fetchRagAnswer(userInput: string) {
    setIsLoading(true);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setInput('');
    setHasAsked(true);
    fetchRagAnswer(input.trim());
  };

  return (
    <section className="relative overflow-hidden bg-gradient-to-br from-background via-primary/5 to-primary/10 min-h-[80vh] flex flex-col items-center justify-center px-4 py-16" style={{ zIndex: 1 }}>
      <div className="max-w-2xl w-full flex flex-col items-center gap-6">
        ...
        {!hasAsked && (
          <>
            <h1 className="text-4xl md:text-5xl font-extrabold text-center mb-2 bg-gradient-to-r from-primary to-primary/80 bg-clip-text text-transparent">
              Build your VO₂max knowledge (RAG)
            </h1>
            <p className="text-lg md:text-xl text-center text-muted-foreground mb-6">
              Ask anything about your uploaded research papers. Answers are based only on your documents.
            </p>
          </>
        )}
        ...
        <form onSubmit={handleSubmit} className="w-full max-w-2xl flex items-center gap-2 bg-card rounded-2xl shadow-lg border border-border px-4 py-4">
          <input
            className="flex-1 border-none bg-transparent text-lg focus:ring-0 focus:outline-none placeholder:text-muted-foreground text-foreground h-14"
            type="text"
            placeholder={hasAsked ? 'Ask another question about your papers...' : 'Ask anything about your research PDFs...'}
            value={input}
            onChange={e => setInput(e.target.value)}
            disabled={isLoading}
            aria-label="Ask anything about your research PDFs"
            autoFocus
          />
          <button
            type="submit"
